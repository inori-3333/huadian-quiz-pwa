...
def main() -> None:
    ...
    with zipfile.ZipFile(args.apk) as apk:
        ...
        required = {"AndroidManifest.xml", "classes.dex", "assets/index.html", "assets/banks-data.js", "assets/assets/data/exam0828.json"}
        ...
        assert attrs["versionCode"][1] == 10203
        assert attrs["versionName"][2] == "1.2.3"
        ...
        for removed_copy in (b'STUDY DESK', b'CURRENT BANK', '把每一道错题'.encode(), '两个题库独立练习'.encode(), '选择题库'.encode()):
            assert removed_copy not in main_js
        assert embedded.startswith(b'// Generated by scripts/embed_banks.py')
    print("Clean APK verified: zero permissions, one embedded bank, no native bridge, 270 questions.")
